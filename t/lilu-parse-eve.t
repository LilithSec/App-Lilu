...
# ===========================================================================
is( $lilu->parse_eve( type => 'suricata', json => { event_type => 'flow' } ), undef, 'a non-alert event is skipped' );
is( $lilu->parse_eve( type => 'suricata', json => {} ),           undef, 'a record with no event_type is skipped' );
is( $lilu->parse_eve( type => 'suricata', json => undef ),        undef, 'undef json is skipped' );
is( $lilu->parse_eve( type => 'suricata', json => 'not a hash' ), undef, 'non-hash json is skipped' );
is( $lilu->parse_eve( type => 'bogus',    json => { event_type => 'alert' } ),
...
# ===========================================================================
{
	...
	is_deeply(
		[ sort keys %{$row} ],
		[ sort @{ $App::Lilu::alert_columns{suricata} } ],
	...
	# flow_id + in_iface, matching Lilith so both compute the same handle
...
# ===========================================================================
{
	...
	is_deeply(
		[ sort keys %{$row} ],
		[ sort @{ $App::Lilu::alert_columns{sagan} } ],
		'sagan row keys match the sagan_alerts column set exactly (single proto)'
	);
	is( scalar( grep { $_ eq 'proto' } @{ $App::Lilu::alert_columns{sagan} } ),
		1, 'proto appears exactly once in the sagan column list' );

	# instance_host is the sensor Lilu runs on; host is the log-originating host
...
# -- cape_submit is the richest source; slug prefers suricata_extract_submit --
{
	...
	is_deeply(
		[ sort keys %{$row} ],
		[ sort @{ $App::Lilu::alert_columns{cape} } ],
...
# -- falls back to suricata_extract_submit, fileinfo.size, and no cape_submit --
{
	...
	my $row = $lilu->parse_eve( type => 'cape', json => $eve, instance => 'cape1', host => 'sensor1' );
...
# -- neither submit source: target comes from row.target --
{
	...
	my $row = $lilu->parse_eve( type => 'cape', json => $eve, instance => 'i', host => 'h' );
...
# the nested detail kept only in raw, the derived event_id, and the ignore knob.
# ===========================================================================

# -- a banish: has ip/dest_ip/ban_time/recidive/country; nested detail in raw --
{
	my $eve = {
		eve_type   => 'baphomet',
		event_type => 'banish',
	...
	is_deeply(
		[ sort keys %{$row} ],
		[ sort @{ $App::Lilu::alert_columns{baphomet} } ],
	...
	# rule name + offender (the ip when present), matching Lilith
	my $expect_eid
...
# -- a found: instance falls back to the record kur when not given --
{
	...
	my $raw = encode_json($eve);
	my $row = $lilu->parse_eve( type => 'baphomet', json => $eve, host => 'sensor1', raw => $raw );
...
is( $lilu->parse_eve( type => 'baphomet', json => { event_type => 'stats' } ),
	undef, 'a baphomet event_type outside the known set is skipped' );

# -- baphomet_event_ignore drops the configured event types --
{
	my $ignoring = App::Lilu->new( dsn => 'dbi:Pg:dbname=test', baphomet_event_ignore => ['noted'] );
	is(
		$ignoring->parse_eve(
			type => 'baphomet',
